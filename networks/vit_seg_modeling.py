...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        encoded_x, attn_weights, features = self.transformer(x)  # (B, n_patch, hidden)
        x = self.decoder(encoded_x, features)
...
class reconstruction(nn.Module):
    def __init__(self,config):
        ...
        self.decoder = DecoderCup(config)
        self.conv = nn.Conv2d(config['decoder_channels'][-1], out_channels=1, kernel_size=3, padding=3 // 2)
        self.upsampling = nn.UpsamplingBilinear2d(scale_factor=1)
    def forward(self,x,features):
        x = self.decoder(x, features)
        logits = self.conv(x)
        return logits

class rotate_cls(nn.Module):
    def __init__(self):
        super(rotate_cls, self).__init__()
        # self.decoder = DecoderCup(config)
